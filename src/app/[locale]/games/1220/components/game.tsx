  ...
  const roll = () => {
    ...
    setTimeout(() => {
      ...
      const results = configs.map(cfg => {
        ...
        const lines: string[] = []
        if (r.drinkSips > 0) lines.push(t('outcomes.drink', { number: cfg.drinkNumber }))
        if (r.giveRawCount > 0) {
          for (const gr of r.giveReasons) lines.push(t('outcomes.give', { label: gr.label }))
          if (r.partialHit) {
            lines.push(t('outcomes.partialGive', { numbers: r.partialNumbers.join(", "), count: formatSips(r.giveEffective) }))
          } else {
            lines.push(t('outcomes.totalGive', { count: formatSips(r.giveEffective) }))
          }
          ...
          {players.map(p => {
            ...
            return (
              <div key={p.id} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {BAND_KEYS.map(b => (
                      <button
                        key={b}
                        onClick={() => updateDraft(p.id, { band: b })}
                        className={cn(
                ...
                {clash && (
                  <p className="text-xs text-amber-400 font-medium">{t('clashWarning')}</p>
                )}
              </div>
            )
          })}
    ...
    <div className="min-h-screen bg-[#07060b] text-white">
          ...
          <div className="flex items-center justify-center gap-6 mb-5">
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <div className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-center">
                <p className="text-[10px] uppercase tracking-widest text-white/35 mb-0.5">{t('sum')}</p>
                <motion.p
                  key={total}
          ...
          {lastRoll && (
            ...
            >
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold uppercase tracking-widest text-white/40">{t('results')}</p>
              ...
              <div className="space-y-2">
                {lastRoll.results.map(r => {
                  ...
                  const allGood = r.text.length === 1 && r.text[0] === t('outcomes.nothing')
                  return (
                    <div
                      key={r.playerId}
                      className={cn(
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] overflow-hidden">
            <details>
              <summary className="flex cursor-pointer items-center justify-between px-4 py-3 text-[11px] font-semibold uppercase tracking-widest text-white/40 hover:text-white/60 transition">
                <span>{t('history', { count: history.length - 1 })}</span>
              </summary>
      ...
      <GameFixedActionBar>
        <button
          onClick={roll}
          disabled={rolling}
          aria-label={rolling ? t('rollAriaRolling') : t('rollAria')}
