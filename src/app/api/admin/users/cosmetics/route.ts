...
/** Seul le fondateur débloque des cosmétiques à la main. */
const isFondateur = (role: string) => normalizeRole(role) === 'fondateur'

export async function GET(request: Request) {
  try {
    await requireRole(isFondateur)
    ...
    return adminErrorResponse(error, 'cosmetics GET')
  }
}

export async function POST(request: Request) {
  try {
    const actor = await requireRole(isFondateur)
    ...
    return adminErrorResponse(error, 'cosmetics POST')
  }
}
