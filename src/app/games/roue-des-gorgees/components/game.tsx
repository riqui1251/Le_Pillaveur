...
export default function Game({ players, onGameEnd, updatePlayerStats, riskLevel, segmentCount }: GameProps) {
  ...
  // Génération égale: 1 SAFE toutes les 3 cases, le reste aléatoire 1..12
  const generateSegments = (_risk: RiskLevel, count: number): Segment[] => {
    const n = Math.max(15, count)
    const arr: Segment[] = []
    for (let i = 0; i < n; i++) {
      const isSafe = (i + 1) % 3 === 0
      const value = isSafe ? 0 : 1 + Math.floor(Math.random() * 12)
      arr.push({ id: `seg-${i}-${value}-${Math.random().toString(36).slice(2,6)}`, label: value === 0 ? 'SAFE' : `${value} gorgées`, value })
    }
    return arr
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="text-sm text-slate-300">Roue générée automatiquement — Risque: <span className="text-slate-200 font-medium">{riskLevel}</span>, Segments: <span className="text-slate-200 font-medium">{segments.length}</span></div>
        <div className="text-xs text-slate-400">Contenu: SAFE et gorgées 1 à 12, avec répétitions. Plus la valeur est élevée, plus le segment est petit.</div>
      </Card>

      {/* Zone de jeu mobile */}
      <Card className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm text-slate-300">Tour de</div>
