...
export default function ClassementPage() {
  ...
  const localizedMetrics = useGameMetricLabels(selectedGame)

  const currentGame = games.find((g) => g.id === selectedGame) ?? games[0]
  const metrics = useMemo(() => localizedMetrics, [localizedMetrics])
      ...
      <div className="relative container mx-auto max-w-3xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8">
        ...
        {activeTab === 'global' && (
          <div className="grid gap-4 sm:grid-cols-2">
            <Section icon={<Trophy className="h-4 w-4" />} title={t('sections.topWins')}>
              {topPlayers.slice(0, 10).length === 0 ? (
                <p className="py-6 text-center text-sm text-white/30">{tCommon('noData')}</p>
              ) : (
                topPlayers.slice(0, 10).map((player, i) => (
                  <PlayerRow
                    ...
                    player={player}
                    index={i}
                    metric={t('metrics.winsShort', { count: player.stats.wins || 0 })}
                  />
                ))
              )}
            </Section>
            <Section icon={<Star className="h-4 w-4" />} title={t('sections.mostActive')}>
              {mostActivePlayers.slice(0, 10).length === 0 ? (
                ...
                mostActivePlayers.slice(0, 10).map((player, i) => (
                  <PlayerRow
                    ...
                    player={player}
                    index={i}
                    metric={t('metrics.gamesShort', { count: player.stats.gamesPlayed || 0 })}
                  />
                ))
              )}
            </Section>
        ...
        {activeTab === 'jeu' && (
          <div className="space-y-4">
            ...
            <Section icon={<span>{currentGame.emoji}</span>} title={currentMetric?.title ?? t('sections.ranking')}>
              {gameRanking.length === 0 ? (
                <p className="py-8 text-center text-sm text-white/30">{t('noDataForGame')}</p>
              ) : (
