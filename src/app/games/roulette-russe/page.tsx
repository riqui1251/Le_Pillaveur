...
import { Trophy, RotateCcw, Beer, Skull, Heart, Home } from 'lucide-react';
  ...
  if (!gameStarted) {
    ...
    return (
      <div className="container mx-auto p-4">
        <h1 className="text-3xl font-bold mb-6 text-center">Roulette Russe</h1>
        ...
        {!gameOver ? (
            ...
            >
              {showResult ? (
                ...
                <Button onClick={pullTrigger} className="w-full">
                  <PlayerName player={gamePlayers[currentPlayerIndex]} /> <span className="ml-2">tire</span>
                </Button>
              )}
            </motion.div>
          </div>
          ...
          <div className="text-center">
            ...
            >
              ...
              <h2 className="text-3xl font-bold mb-2">Victoire !</h2>
              {winner && (
                <p className="text-xl text-muted-foreground mb-6">
                  <span className="font-bold text-foreground">
                    <PlayerName player={winner} />
                  </span> est le dernier survivant !
                </p>
              )}
            </motion.div>
            
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-3">Récapitulatif des gorgées :</h3>
                  ...
                  .map((player) => (
                    <div key={player.id} className="flex items-center justify-between p-3 bg-yellow-100 border-2 border-yellow-500 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-yellow-800">{player.drinks} gorgées</span>
                      </div>
                    </div>
                  ))}
                {gamePlayers
                  ...
                  .map((player) => (
                    <div key={player.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <Beer className="w-4 h-4" />
                        <span className="font-semibold">{player.drinks} gorgées</span>
                      </div>
                    </div>
                  ))}
              </div>
